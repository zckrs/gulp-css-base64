...
describe('gulp-css-base64', function () {
    ...
    describe('in buffer mode', function () {
        ...
        it('should use option preProcess before encode with remote resource', function (done) {
            // create the fake file
            var fakeFile = new File({
                contents: new Buffer('.button_alert{background:url(http://www.google.com/favicon.ico) no-repeat 4px 5px;padding-left:12px;font-size:12px;color:#888;text-decoration:underline}')
            });
            ...
            var stream = base64({
                preProcess : function(file, callback) {
                    file.path = 'test/fixtures/image/very-very-small.png';
                    file.contents = fs.readFileSync('test/fixtures/image/very-very-small.png');
                    return callback(file);
                }
            });
